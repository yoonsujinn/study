...
document.addEventListener("DOMContentLoaded", function () {
  ...
  slideBtn_dotWrap.children[0].classList.add('on')
  let slidDot = [...slideBtn_dotWrap.children]; // DOM API에서 제공하는 유사배열이라서 ...로 풀어서 써야함
    ...
    slidDot.forEach((ele,idx) => {
      ...
      ele.addEventListener('click',() => {
        ...
        pageWrapLi.forEach(ele => {ele.classList.remove('on')})
        pageWrapLi[idx].classList.add('on')
      })
    });
    ...
    next.addEventListener('click', ()=> {
      ...
      pageWrapLi.forEach(ele => {ele.classList.remove('on')})
      pageWrapLi[nowDotIdx+1].classList.add('on')
      slidDot[nowDotIdx].classList.remove('on')
      slidDot[nowDotIdx+1].classList.add('on')
    ...
    prev.addEventListener('click', ()=> {
      ...
      slidDot.forEach((ele,idx) => {
        ...
        console.log(nowDotIdx2)

      pageWrapLi.forEach(ele => {ele.classList.remove('on')})
      pageWrapLi[nowDotIdx2].classList.add('on')
